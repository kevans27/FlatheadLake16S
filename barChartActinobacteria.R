...
targetLine <- "Actinobacteria"
seqTargs <- as.character(proktax[grep(targetLine, proktax$class), "seqID"])

targetFrameLarge <- largeAbund[rownames(largeAbund) %in% seqTargs,]
targetFrameSmall <- smallAbund[rownames(smallAbund) %in% seqTargs,]

actinoFrame <- merge(targetFrameLarge, targetFrameSmall, by = 0, all = TRUE)
actinoFrame[is.na(actinoFrame)] <- 0
rownames(actinoFrame) <- actinoFrame$Row.names
actinoFrame$Row.names <- NULL
actinoFrame$Sequence.x <- NULL
actinoFrame$Sequence.y <- NULL

mainVal <- colSums(actinoFrame)
trimmedFram <- actinoFrame
miniMat <- sweep(trimmedFram, 2, mainVal, FUN = '/')
miniMat$total <- rowSums(miniMat)

actinoAbund <- miniMat[order(miniMat$total, decreasing = TRUE), ]
actinoAbund$total <- NULL

relabundTax <- function(countTable, prokLevel){
  prokSubTax <- proktax[,c("seqID", prokLevel)]
  prokSubTax[,"replaced"] <- gsub("\\s*\\([^\\)]+\\)","",
                                  as.character(prokSubTax[,prokLevel]))
...
actinoLine <- relabundTax(actinoAbund, "lineage")
actinoClade <- relabundTax(actinoAbund, "clade")
actinoTribe <- relabundTax(actinoAbund, "tribe")
targFrame <- actinoTribe
...
rownames(colorSorting)[rownames(colorSorting) == "otherProks"] <- "Other Actinobacteria"
...
png("FigBin/allDepthsBarChartActinobacteriaTribePlus.png", width = 2400, height = 1200)
plot.new()
...
abline(v = 4.4, lty = 2, col = "black", lwd = 5)
abline(v = 21, lty = 2, col = "black", lwd = 5)
mtext("% of Actinobacteria 16S rRNA Genes in Sample", 2, cex = 2.5, font = 2, las = 0, 
      line = 8, adj = 0.5)
...
legend("topright", inset=c(-0.3,0), legend = listOfPhyls[1:9], 
       fill = unique(sortedColors[1:9]), xpd = NA, cex = 1.8, 
       title = "Actinobacteria Tribe",
       box.lwd = 2)
